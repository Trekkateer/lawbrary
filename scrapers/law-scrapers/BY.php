                ...
                foreach ($laws as $law) {
                    ...
                    if (isset($IDs[explode('-', explode('/documents/', $law['url'])[1])[0]]) || str_contains('normal-0-false-false-false-microsoftinternetexplorer4-', $law['url'])) {
                        ...
                        $explodeURL = explode('-', strtr(explode('/documents/', $law['url'])[1], array('-no-'=>'-', '-np-'=>'np-', 'executive-order'=>'executiveorder', 'normal-0-false-false-false-microsoftinternetexplorer4-'=>'', '-st1behaviorurlieooui-'=>'-', '-st1behaviorurlieooui-'=>'-')));
                            ...
                            $ID = $country.'-'.$IDs[$explodeURL[0]].preg_replace('/[A-Za-z]/', '', $explodeURL[1].$ID); echo $ID.'</br>';
                        //Gets other values
                        $enactDate = date('Y-m-d', $law['date']); $enforceDate = $enactDate; $lastactDate = $enactDate;
                        $name = trim($law['description']);
                        //Gets the regime
                        switch(true) {
                            case strtotime('25 March 1918') < strtotime($enactDate) && strtotime($enactDate) < strtotime('1 January 1919'):
                                $regime = 'The Democratic Republic of Belarus';
                                break;
                            case strtotime('1 January 1919') < strtotime($enactDate) && strtotime($enactDate) < strtotime('25 August 1991'):
                                $regime = 'The Belarusian S.S.R.';
                                break;
                            case strtotime('25 August 1991') < strtotime($enactDate) && strtotime($enactDate) <= strtotime(date('d M Y')):
                                $regime = 'The Republic of Belarus';
                                break;
                        }
                        //Gets the rest of the values
                        $summary = trim($law['comment']);
                        $type = $types[$law['category'][0]['name']]; $status = 'Valid';
                        ...
                        $SQLFind = "SELECT * FROM `laws".strtolower($country)."` WHERE `id`='".$ID."'";
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                //Updates the table
                                $SQL2 = "UPDATE `laws".strtolower($country)."` SET `name`='".$name."', `summary`='".$summary."', `source`='".$source."' WHERE `ID`='".$ID."'";
                                ...
                                /*if (isset(json_decode($row['source'], true)[$lang])) {
                                    if ($source !== json_decode($row['source'], true)[$lang]) {
                                        ...
                                        //Creates correct SQL
                                        $SQL2 = "INSERT INTO `laws".strtolower($country)."`(`enactDate`, `enforceDate`, `lastactDate`, `ID`, `name`, `regime`, `summary`, `type`, `status`, `source`) 
