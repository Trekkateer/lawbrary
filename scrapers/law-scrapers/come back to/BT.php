<html><body>
    <?php //Getting the ID is impossible and language is very difficult
        ...
        $lawParagraghs = $html_dom->find('div.is-layout-constrained.entry-content.wp-block-post-content')[0]->find('p');
        foreach ($lawParagraghs as $lawParagragh) {
            ...
            foreach ($laws as $law) {
                ...
                //Gets the values
                $ID = $country.'-'.IDs[trim(explode('.pdf', explode('/uploads/', $law_dom->find('a')[0]->href)[1])[0])];
                ...
                //Creates SQL
                $SQL2 = "INSERT INTO `laws".strtolower($country)."`(`ID`, `name`, `regime`, `type`, `status`)
        ...
        for ($i = 4; $i <= sizeof($lawParagraghs); $i++) {
            ...
            foreach ($laws as $law) {
                if (str_contains($law, '<a href="')) {
                    ...
                    //Gets the values
                    $ID = $country.'-'.IDs[explode('.pdf', explode('/uploads/', $law_dom->find('a')[0]->href)[1])[0]];
