<html><body>
    <?php //!!There is no consistant ID !!Many laws are missing a source page
        //Settings
        $test = true; $country = 'LK';
        ...
        foreach (array('si', 'ta', 'en') as $lang) {
            ...
            $limit = $limit ?? explode("'", end($html_dom->find('a.pagenav'))->onclick)[1]; echo $limit;
            ...
            for ($offset = $start; $offset <= $limit; $offset += $step) {
                ...
                foreach ($laws as $lawNum => $law) {//echo $law;
                    $enactDate = str_replace('-', '/', explode(': ', $law->find('div.nTabber_content')[0]->find('div.con_box')[1]->plaintext)[1]); $enforceDate = $enactDate;
                    $ID = $country.'-'.strtr(trim(explode(':', $law->find('div.acts_box_top')[0]->find('a.nTabber_plus')[0]->plaintext)[0]), array('/'=>'', 'Chap '=>'CHAP'));
                    $name = trim(explode(':', $law->find('div.acts_box_top')[0]->find('a.nTabber_plus')[0]->plaintext)[1]);
                    $type = 'Act';
                        if (str_contains($name, 'සංශෝධන')) {$type = $type.' Amendment';}
                        if (isset($law->find('img[src="https://parliament.lk/images/c-ammend.png"]')[0])) {$type = 'Constitutional Amendment'; $ID = 'LK-CON'.str_replace('/', '', $enactDate);}
                    $status = 'Valid';
                    $source = $law->find('div.nTabber_content')[0]->find('div.con_box')[2]->find('a.act_down')[0]->href ?? 'NULL'; $PDF = $source;
