    ...
    <?php //Link is broken
        ...
        for ($page = $start; $page < $limit; $page++) {
            ...
            foreach ($regulations as $regulation) {
                ...
                $ID = $LBpage.':'.str_replace('/', '', $regulation['name']);
                $name = $regulation['title'];
                //Gets regime and country
                $country = '["IS"]';
                switch(true) {
                ...
                //Makes sure there are no quotes in the title (Using Wikipedia for Icelandic punctuation standards)
                ...
                $SQL2 = "INSERT INTO `laws".strtolower($LBpage)."`(`enactDate`, `enforceDate`, `lastactDate`, `ID`, `name`, `country`, `regime`, `origin`, `type`, `status`, `source`) 
                            VALUES ('".$enactDate."', '".$enforceDate."', '".$lastactDate."', '".$ID."', '".$name."', '".$country."', '".$regime."', '".$origin."', '".$type."', '".$status."', '".$source."')"; echo $SQL2.'<br/>';
