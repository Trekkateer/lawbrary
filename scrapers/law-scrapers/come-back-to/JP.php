            ...
            foreach ($alphabet as $letter) {
                ...
                foreach($laws as $law) {
                    ...
                    if ($vals['Revision'] === '제정' || $vals['Revision'] === 'New Enactment') {
                        ...
                        $SQL = "SELECT * FROM `laws".strtolower($LBpage)."` WHERE `ID`='".$vals['ID']."'";
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                $SQL2 = "UPDATE `laws".strtolower($LBpage)."` SET `name`='".$name."', `source`='".$source."', `type`='".$type."', `origin`='".$origin."' WHERE `ID`='".$vals['ID']."'";
                            }
                        } else {
                            ...
                            //Creates SQL
                            $SQL2 = "INSERT INTO `laws".strtolower($LBpage)."`(`enactDate`, `enforceDate`, `ID`, `name`, `type`, `origin`, `status`, `source`)
