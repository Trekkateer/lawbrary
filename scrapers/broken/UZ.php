<?php //Uzbekistan
    //!!The only way to get the correct page is to have the right _VIEWSTATE, which I don't know how to get!!

    //Settings
    $test = true; $scraper = 'UZ';
    ...
    $saveDate = date('Y-m-d'); $country = '["UZ"]';
    $publisher = '{"en":"The Ministry of Justice of the Republic of Uzbekistan", "uz":"Adliya vazirligi"}';
    ...
    foreach (array('uz'=>4/*, 'ru'=>1, 'en'=>3*/) as $lang => $langCode) {
        ...
        for ($page = $start; $page <= $limit; $page++) {
            //Processes the data
            $html_dom->load($HTTP_Call('https://lex.uz/'.$lang.'/search/nat?lang='.$langCode, $str_pad($page, 2, '0', STR_PAD_LEFT)));
            ...
            foreach($laws as $law) {
                ...
                $ID = $scraper.':'.strtr(end(explode(' ', $law->find('span.badge.badge-pill.badge-nine', 0)->plaintext)), array('-'=>'', '’'=>'', '‘'=>''));
                $name = fixQuotes($law->find('div.dd-table__main-left-desc', 0)->find('a.lx_link', 0)->plaintext, $lang);
                $regime = strtotime($enactDate) < strtotime('1991-08-31') ? '{"uz":"O’zbekiston SSR", "en":"The Uzbek SSR"}':'{"uz":"O’zbekiston Respublikasi", "en":"The Republic of Uzbekistan"}';
                ...
                if ($result->num_rows > 0) {
                    ...
                    //Creates SQL
                    $SQL2 = "INSERT INTO `".strtolower($scraper)."`(`enactDate`, `enforceDate`, `lastactDate`, `saveDate`, `ID`, `name`, `country`, `regime`, `origin`, `type`, `status`, `source`, `PDF`)
                            VALUES ('".$enactDate."', '".$enforceDate."', '".$lastactDate."', '".$saveDate."', '".$ID."', '".$name."', '".$country."', '".$regime."', '".$origin."', '".$type."', ".$status."', '".$source."', '".$PDF."')";
                }
