<?php //Getting the ID is impossible and language is very difficult
    ...
    //Sets static values
    $saveDate = date('Y-m-d'); $country = '["BT"]';
    $type = 'Law'; $status = 'Valid';
    $publisher = '{"en":"The Office of the Attorney General","dz":"ཁྲིམས་སྡོད་རྒྱལ་ཁབ་གི་ལས་ཁུངས"}';
    ...
    foreach ($lawParagraghs as $lawParagragh) {
        ...
        foreach ($laws as $law) {
            ...
            $name = fixQuotes(trim($law_dom->find('span', 0)->plaintext), $lang);
            ...
            //Creates SQL
            $SQL2 = "INSERT INTO `".strtolower($scraper)."`(`saveDate`, `ID`, `name`, `country`, `regime`, `publisher`, `type`, `status`)
                    VALUES ('".$saveDate."', '".$ID."', '".$name."', '".$country."', '".$regime."', '".$publisher."', '".$type."', '".$status."')";
    ...
    for ($i = 4; $i <= sizeof($lawParagraghs); $i++) {
        ...
        foreach ($laws as $law) {
            if (str_contains($law, '<a href="')) {
                ...
                $enactDate = $enforceDate = $lastactDate = end(explode(' ', trim($law_dom->find('a', 0)->plaintext))).'-01-01';
                $name = fixQuotes(trim($law_dom->plaintext, ' ('), $lang);
            }
        }
    }*/
