<?php //Micronesia
    //!!Not all laws have valid dates or links!!
    ...
    $saveDate = date('Y-m-d'); $country = '["FM"]';
    $type = 'Law'; $status = 'Valid';
    $regime = '{"en":"The Federated States of Micronesia"}';
    $publisher = '{"en":"The Congress of the Federated States of Micronesia"}';
    ...
    foreach (array('Public Law'=>10,'Resolution'=>20,'Bill'=>15) as $type => $start) {
        ...
        for ($congress = $start; $congress <= $limit; $congress++) {
            //Processes the data
            $dom = file_get_html('https://www.cfsm.gov.fm/'.$ordinalize($congress).'-'.strtr(strtolower($type), [' '=>'-']).'s/');
            $laws = $dom->find('table[id^="tablepress"]', 0)->find('tbody.row-hover', 0)->find('tr');
            foreach($laws as $lawNum => $law) {
                ...
                $name = fixQuotes(trim($law->find('td.column-3', 0)->plaintext), 'en');
                //TODO: Figure out how to tell if the law is an amendment and what it amends
                $status = 'Valid';
                $source = $law->find('td.column-1', 0)->find('a', 0)->href;
                ...
                $SQL2 = "INSERT INTO `".strtolower($scraper)."`(`enactDate`, `enforceDate`, `lastactDate`, `saveDate`, `ID`, `country`, `regime`, `name`, `type`, `status`, `source`, `PDF`) 
                        VALUES ('".$enactDate."', '".$enforceDate."', '".$lastactDate."', '".$saveDate."', '".$ID."', '".$country."', '".$regime."', '".$name."', '".$type."', '".$status."', '".$source."', '".$PDF."')";
