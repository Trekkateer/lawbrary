<?php //!! runs out of memory
    ...
    //Changes the execution time to infinity
    if (!$test) ini_set('max_execution_time', 0);
    ...
    $saveDate = date('Y-m-d');
    $publisher = '{"vi":"LuatVietnam","en":"LuatVietnam"}';
    ...
    for ($page = $start[$lang]; $page <= $limit[$lang]; $page++) {
        ...
        foreach ($laws as $law) {
            ...
            $name = fixQuotes(trim(str_replace(array_keys($sanitizeName[$lang]), array_values($sanitizeName[$lang]), $law->find('div.doc-clumn2', 0)->find('div.post-doc', 0)->find('div.post-type-doc', 0)->find('a', 0)->plaintext), ' ​.'), $lang);
            //Gets the regime
            $country = array("VN");
            switch (true) {
    ...
    for ($page = $start[$lang]; $page <= $limit[$lang]; $page++) {
        ...
        foreach($laws as $law) {
            ...
            $name = fixQuotes(trim(str_replace(array_keys($sanitizeName[$lang]), array_values($sanitizeName[$lang]), $law->find('div.col1-document', 0)->find('div.post-document', 0)->find('a', 0)->plaintext)), $lang);
            //Gets the regime
            $country = array("VN");
            switch (true) {
                case strtotime($enactDate) <= strtotime('today'):
            ...
            if ($result->num_rows > 0) {
                ...
                //Creates SQL
                $SQL2 = "INSERT INTO `".strtolower($LBpage)."`(`enactDate`, `enforceDate`, `lastactDate`, `saveDate`, `ID`, `name`, `country`, `regime`, `publisher`, `type`, `status`, `source`)
