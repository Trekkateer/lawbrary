<?php //Armenia
    ...
    $saveDate = date('Y-m-d'); $country = '["AM"]'; $status = 'Valid';
    $publisher = '{"hy":"ՀՀ Ազգային ժողով", "en":"The National Assembly of the Republic of Armenia"}';

    //Loops through languages
    foreach (array('arm'=>'hy', 'rus'=>'ru', 'eng'=>'en', 'frn'=>'fr') as $locale=>$lang) {
        //Gets the HTML 
        $dom = file_get_html('http://www.parliament.am/legislation.php?sel=alpha&lang='.$locale);

        //Processes the data in the table
        $laws = $dom->find('a[target="_new"]');
        foreach ($laws as $law) {
            //Gets Hyperref
            ...
            if ($source !== 'http://www.parliament.am#') {//Makes sure we can get the source
                ...
                $ID = $scraper.':'.explode('&lang', explode('ID=', $source)[1])[0];
                ...
                $name = fixQuotes(explode(' ('.substr(end(explode(' (', $law->plaintext)), 0, 10), $law->plaintext)[0], $lang);
                ...
                if ($law->class === "blue_sm_11") {
                    ...
                    $SQL21 = "SELECT * FROM `".strtolower($scraper)."` WHERE `ID`='".$lastID."'";
                    $result = $conn->query($SQL21);
                    $row = $result->fetch_assoc();
                    $amendedBy = json_decode($row['amendedBy'], true);
                    $amendedBy[] = $ID;
                    $amendedBy = json_encode($amendedBy, JSON_UNESCAPED_UNICODE);
                    $SQL21 = "UPDATE `".strtolower($scraper)."` SET `amendedBy`='".$amendedBy."' WHERE `ID`='".$lastID."'";
                    echo $SQL21.'<br/>'; $conn->query($SQL21);
                    //Changes the lastactDate
                    $SQL23 = "UPDATE `".strtolower($scraper)."` SET `lastactDate`='".$enactDate."' WHERE `ID`='".$lastID."'";
                    echo $SQL23.'<br/>'; $conn->query($SQL23);
                } else {$amends = 'NULL';}
                ...
                if ($result->num_rows > 0) {
                    ...
                    //Creates SQL
                    $SQL2 = "INSERT INTO `".strtolower($scraper)."`(`enactDate`, `enforceDate`, `lastactDate`, `saveDate`, `ID`, `name`, `country`, `regime`, `publisher`, `type`, `amends`, `status`, `source`)
                            VALUES ('".$enactDate."', '".$enforceDate."', '".$lastactDate."', '".$saveDate."', '".$ID."', '".$name."', '".$country."', '".$regime."', '".$publisher."', '".$type."', ".$amends.", '".$status."', '".$source."')";
                }
