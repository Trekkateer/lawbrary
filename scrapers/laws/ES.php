<?php //Spain
    ...
    ### TODO: ###
    // Implement crawling the specific law pages. Get PDF and types
    // Make a way for a law to have multiple origins
    // Implement ammendment support

    //Settings
    $test = false; $scraper = 'ES';
    $start = 0;//Which law to start from
    $step = 500;//How many laws there are on each page. 1000 and 2000 will break the program
    $limit = NULL;//How many laws to get. The current limit is 243,143
    ...
    $saveDate = date('Y-m-d'); $country = '["es"]'; $status = 'Valid';
    $publisher = '{"es":"Boletín Oficial del Estado", "en":"The Official State Gazette", "ca":"Butlletí Oficial de lꞌEstat", "gl":"Boletín Oficial do Estado", "eu":"Estatuko Aldizkari Ofiziala", "fr":"Journal officiel de lꞌÉtat"}';

    //Gets the limit
    $dom = file_get_html('https://boe.es/buscar/legislacion.php?accion=Mas&id_busqueda=cEU0a2k0TlQrU0podUhsUkFnVUkrY212aitpQWdOL0hvYmY2QUlJci82TllCZXJmT0ZUN2ZOT2R6b2hXekJtZFc2UkpJUHBlMGY0aitFY0JJc2ZhZjN0MFMzZGptWEdlU3JDL3k0YWEwQXlvRlZPbUxXV2cydXBpL2x6ak84RmZOYWJoejNZWGdVYlhTOTdndEdzcjdyNTcwalZObk9mL1RYdWtEZ1o0bDRBPQ,,-'.$start.'-'.$step);
    $limit = $limit ?? str_replace('.', '', explode(' de ', $dom->find('div.paginar', 0)->plaintext)[1]);
    //Loops through the pages
    for ($offset = $start; $offset < $limit; $offset += $step) {
        //Processes the data
        $dom = file_get_html('https://boe.es/buscar/legislacion.php?accion=Mas&id_busqueda=cEU0a2k0TlQrU0podUhsUkFnVUkrY212aitpQWdOL0hvYmY2QUlJci82TllCZXJmT0ZUN2ZOT2R6b2hXekJtZFc2UkpJUHBlMGY0aitFY0JJc2ZhZjN0MFMzZGptWEdlU3JDL3k0YWEwQXlvRlZPbUxXV2cydXBpL2x6ak84RmZOYWJoejNZWGdVYlhTOTdndEdzcjdyNTcwalZObk9mL1RYdWtEZ1o0bDRBPQ,,-'.$offset.'-'.$step);
        $laws = $dom->find('div#contenido', 0)->find('div.listadoResult', 0)->find('ul li.resultado-busqueda');
        foreach ($laws as $law) {
            //Gets the values
            $enactDate = $enforceDate = $lastactDate = date('Y-m-d', strtotime(str_replace('/', '-', end(explode(' ', trim($law->find('h3', 0)->plaintext, ' )'))))));
            $LBPage = $origins[explode(' (', $law->find('h3', 0)->plaintext)[0]][1];
                $ID = $LBPage.':'.str_replace('-', '', explode('Ref. ', $law->find('a', 0)->title)[1]);
            $name = fixQuotes(trim($law->find('p', 0)->plaintext, ' .'), 'es');
            //Gets the regime
            $regime = '{"en":"The Kingdom of Spain", "es":"El Reino de España", "ca":"El Regne dꞌEspanya", "gl":"O Reino de España", "eu":"Espainiako Erresuma", "fr":"Le Royaume dꞌEspagne"}';
            //Gets the type
            $type = ''; $isAmmend = 0;
            if (str_starts_with($name, 'Corrección') || str_starts_with($name, 'Enmienda') || str_starts_with($name, 'Modificación') || str_starts_with($name, 'Modificaciones') || str_starts_with($name, 'Reforma') || str_starts_with($name, 'Texto enmendado') || str_starts_with($name, 'Textos enmendados')) {$isAmmend = 1;} else {$isAmmend = 0;}
            foreach ($types as $typeES => $typeEN) {
                $typeES = explode('Corrección de errores del ', $name)[1] ?? explode('Enmienda del ', $name)[1] ?? explode('Modificación del ', $name)[1] ?? explode('Modificaciones del ', $name)[1] ?? explode('Reforma del ', $name)[1] ?? explode('Texto enmendado del ', $name)[1] ?? explode('Textos enmendados ', $name)[1] ?? $name;
                if (str_starts_with($typeES, $typeEN.' ') || str_starts_with($typeES, $typeEN.', ')) {
                    $type = $typeEN;
                    break;
                }
            } if ($type == '') {$type = 'Law';}
            ...
            $SQL2 = "INSERT INTO `".strtolower($scraper)."`(`enactDate`, `enforceDate`, `lastactDate`, `saveDate`, `ID`, `name`, `country`, `regime`, `publisher`, `type`, `status`, `origin`, `source`)
                    VALUES ('".$enactDate."', '".$enforceDate."', '".$lastactDate."', '".$saveDate."', '".$ID."', '".$name."', '".$country."', '".$regime."', '".$publisher."', '".$type."', '".$status."', '".$origin."', '".$source."')";
