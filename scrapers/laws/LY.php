<?php  //Libya
    ...
    //TODO: There are way more pages for every type of law. Find a way to get them all

    //Settings
    $test = false; $scraper = 'LY';
    ...
    $saveDate = date('Y-m-d'); $country = '["LY"]';
    $publisher = '{"ar":"المجمع القانوني", "en":"The Law Society of Libya"}';
    ...
    foreach (array('ar'=>'', 'en'=>'en/') as $lang => $directory) {
        ...
        switch ($lang) {
            case 'ar':
                $limit[$lang] = $limit[$lang] ?? file_get_html('https://www.lawsociety.ly/'.$directory)->find('#search-filter-results-52', 0)->find('ul.uk-pagination.uk-flex-center.uk-margin-medium-top', 0)->find('li')[9]->plaintext;
                break;
            case 'en':
        ...
        for ($page = $start[$lang]; $page <= $limit[$lang]; $page++) {echo 'Page: '.$page.'<br/>';
            ...
            $laws = file_get_html('https://www.lawsociety.ly/'.$directory.'page/'.$page.'/?sf_paged='.$page)->find('table#table', 0)->find('tbody', 0)->find('tr.el-item');
            foreach ($laws as $law) {
                //Gets values
                $enactDate = $enforceDate = $lastactDate = explode(' ', trim(strtr($law->find('td', 5)->plaintext, $dataNames)))[2].$months[explode(' ', trim(strtr($law->find('td', 5)->plaintext, $dataNames)))[1]].pad_str(explode(' ', trim(strtr($law->find('td', 5)->plaintext, $dataNames)))[0], 2, '0', STR_PAD_LEFT);
                $ID = $scraper.':'.explode(' ', trim(strtr($law->find('td', 5)->plaintext, $dataNames)))[2].pad_str(strtr($law->find('td', 3)->plaintext, $dataNames), 4, '0', STR_PAD_LEFT);
                //Gets regime
                switch(true) {
                    case strtotime($enactDate) < strtotime('5 November 1911'):
                        $regime = '{"ar":"العهد العثماني لطرابلس", "en":"Ottoman Tripolitania"}';
                        break;
                    case strtotime('5 November 1911') < strtotime($enactDate) && strtotime($enactDate) < strtotime('10 February 1947'):
                        $regime = '{"ar":"ليبيا الإيطالية", "en":"Italian Libya"}';
                        break;
                    case strtotime('10 February 1947') < strtotime($enactDate) && strtotime($enactDate) < strtotime('24 December 1951'):
                        $regime = '{"ar":"فترة الاحتلال", "en":"Allied Occupation of Libya"}';
                        break;
                    case strtotime('24 December 1951') < strtotime($enactDate) && strtotime($enactDate) < strtotime('1 September 1969'):
                        $regime = '{"ar":"المملكة الليبية", "en":"The Kingdom of Libya"}';
                        break;
                    case strtotime('1 September 1969') < strtotime($enactDate) && strtotime($enactDate) <= strtotime('2 March 1977'):
                        $regime = '{"ar":"الجمهورية العربية الليبية", "en":"The Libyan Arab Republic"}';
                        break;
                    case strtotime('2 March 1977') < strtotime($enactDate) && strtotime($enactDate) <= strtotime('23 October 2011'):
                        $regime = '{"ar":"الجماهيرية العربية الليبية الشعبية الاشتراكية", "en":"The Socialist People’s Libyan Arab Jamahirya"}';
                        break;
                    case strtotime('23 October 2011') < strtotime($enactDate) && strtotime($enactDate) <= strtotime(date('today')):
                        $regime = '{"ar":"دولة ليبيا", "en":"The State of Libya"}';
                        break;
                }
                //Gets the rest of the values
                $name = fixQuotes(trim(strtr($law->find('td', 0)->plaintext, array('Title:'=>'', 'العنوان: '=>''))), $lang);
                $type = $types[trim(strtr($law->find('td', 2)->plaintext, $dataNames))];
                //$isAmend = str_contains($law->find('td', 4)->plaintext, 'معدل') ? 1:0;
                $status = $statuses[trim(strtr($law->find('td', 4)->plaintext, $dataNames))];
                $origin = $origins[trim(strtr($law->find('td', 1)->plaintext, $dataNames))];
                $source = strtr($law->find('td', 0)->find('a', 0)->href, "'", "%27");
