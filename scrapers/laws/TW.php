<?php //Taiwan
    ...
    $saveDate = date('Y-m-d'); $country = '["TW"]';
    $regime = '{"zh":"中華民國", "en":"The Republic of China"}';
    $publisher = '{"zh":"全國法規資料庫", "en":"The Laws and Regulations Database of the Republic of China"}';

    //Loops through languages
    foreach (array(''=>'zh'/*, 'ENG/'=>'en'*/) as $locale => $lang) {
        //Gets the limit
        $dom = file_get_html('https://law.moj.gov.tw/'.$locale.'News/NewsList.aspx?psize='.$step);
        $limit = $limit ?? explode('&', explode('page=', $dom->find('#hlPage', 3)->href)[1])[0];

        //Loops through the pages
        for ($page = $start; $page <= $limit; $page++) {
            //Processes the data
            $dom = file_get_html('https://law.moj.gov.tw/'.$locale.'News/NewsList.aspx?page='.$page.'&psize='.$step);
            $laws = $dom->find('table.table.table-hover.tab-list.tab-news', 0)->find('tbody', 0)->find('tr');
            foreach ($laws as $law) {
                //Gets the type
                $type = $types[trim($law->find('td', 2)->plaintext)];
                if ($type !== 'Draft' && $type !== 'Local Regulation') {
                    //Gets the rest of the values
                    $enactDate = $enforceDate = $lastactDate = caltrans($law->find('td', 1)->plaintext, $lang);
                    $ID = $scraper.':'.explode('&', (explode('id=', strtolower($law->find('td', 3)->find('a', 0)->href))[1] ?? explode('/Law/LawSearch/LawInformation/', $law->find('td', 3)->find('a', 0)->href)[1] ?? explode('/Law/LawSearch/LawInformation?sysNumber=', $law->find('td', 3)->find('a', 0)->href)[1] ?? explode('https://www.stat.gov.tw/News.aspx?n', $law->find('td', 3)->find('a', 0)->href)[1]))[0];
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        $SQL2 = "INSERT INTO `".strtolower($scraper)."`(`enactDate`, `enforceDate`, `lastactDate`, `saveDate`, `ID`, `name`, `country`, `regime`, `publisher`, `origin`, `type`, `status`, `source`)
                                VALUES ('".$enactDate."', '".$enforceDate."', '".$lastactDate."', '".$saveDate."', '".$ID."', '".$name."', '".$country."', '".$regime."', '".$publisher."', ".$origin.", '".$type."', '".$status."', '".$source."')";
                    }
