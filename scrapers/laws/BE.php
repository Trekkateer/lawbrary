<?php //Belgium
    ...
    $saveDate = date('Y-m-d'); $status = 'Valid';
    $publisher = '{"nl":"Federale Overheidsdienst Justitie", "fr":"Service public fédéral Justice", "de":"Föderaler Öffentlicher Diest Justiz", "en":"Federal Public Service Justice"}';
        ...
        foreach ($natures[$lang] as $natureBE => $natureUS) {
            ...
            for ($page = $start[$lang]; $page <= $limit[$lang]; $page++) {
                ...
                foreach ($laws as $law) {
                    ...
                    $name = fixQuotes(trim(explode('. - ', $nameLine)[1], ' .'), $lang);
                    //Gets the regime
                    switch(true) {
                    ...
                    //Gets the link
                    $source = strtr('https://www.ejustice.just.fgov.be/cgi_wet/'.$law->find('div.list-item--button', 0)->find('a', 0)->href, array("'"=>"%27"));
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        $SQL2 = "INSERT INTO `".strtolower($LBpage)."`(`enactDate`, `enforceDate`, `lastactDate`, `saveDate`, `ID`, `name`, `country`, `regime`, `origin`, `publisher`, `type`, `status`, `topic`, `source`) 
                                VALUES ('".$enactDate."', '".$enforceDate."', '".$lastactDate."', '".$saveDate."', '".$ID."', '".$name."', '".$country."', '".$regime."', ".$origin.", '".$publisher."', '".$type."', '".$status."', ".$topic.", '".$source."')";
                    }
