<?php
    ...
    $saveDate = date('Y-m-d'); $country = '["SE"]';
    $publisher = '{"sv":"Sveriges Riksdag", "en":"The Swedish Parliament"}';
    ...
    for ($page = $start; $page <= $limit; $page++) {
        ...
        foreach ($laws as $law) {
            //Interprets the data
            $dom->load($law['statusRow']);
                $enactDate = $lastactDate = trim($dom->find('dd', 0)->plaintext ?? explode('-', $law['id'])[1].'-01-01');
            ...
            $name = fixQuotes(trim(str_replace(array_keys($sanitizeName), array_values($sanitizeName), $law['title']), ' -;,'), 'sv');
            $summary = fixQuotes(trim(strtr(trim($law['summary'] ?? 'NULL'), array("\n" => " ", "\r" => " ", "\t" => " ")), ' -;,'), 'sv');
            ...
            //Creates SQL
            $SQL2 = "INSERT INTO `".strtolower($scraper)."`(`enactDate`, `enforceDate`, `lastactDate`, `saveDate`, `ID`, `name`, `country`, `regime`, `origin`, `publisher`, `summary`, `topic`, `type`, `status`, `source`, `PDF`) 
                    VALUES ('".$enactDate."', '".$enforceDate."', '".$lastactDate."', '".$saveDate."', '".$ID."', '".$name."', '".$country."', '".$regime."', ".$origin.", ".$publisher.", ".$summary.", ".$topic.", '".$type."', '".$status."', '".$source."', ".$PDF.")"; echo $page.'. '.$SQL2.'<br/><br/>';
