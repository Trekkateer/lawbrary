<?php //Lithuania
    ...
    $saveDate = date('Y-m-d'); $country = '["LT"]';
    $publisher = '{"lt":"Lietuvos Seimas", "en":"The Parliament of Lithuania"}';
    ...
    foreach($laws as $law) {//echo $law;
        ...
        } else if ($title_dom->find('a', 0) != null) {
            //Adds value for Hyperref
        ...
        $values['Enactment Date'] = $values['Enforcement Date'] = $lastactDate = date('Y-m-d', strtotime($values['Enactment Date']));
        if (!$values["Institutional ID"]) {$ID = $scraper.':'.$pageID;} else {$ID = $scraper.':'.$values['Institutional ID'];}

        //Sets the regime
        $regime = '{"en":"The Republic of Lithuania", "lt":"Lietuvos Respublika"}';
        ...
        $values['Title'] = fixQuotes($values['Title'], 'lt');
        ...
        $SQL2 = "INSERT INTO `".strtolower($scraper)."`(`enactDate`, `enforceDate`, `lastactDate`, `saveDate`, `ID`, `name`, `country`, `regime`, `publisher`, `type`, `status`, `source`) 
            VALUES ('".$values['Enactment Date']."', '".$values['Enforcement Date']."', '".$lastactDate."', '".$saveDate."', '".$ID."', '".$values['Title']."', '".$country."', '".$regime."', '".$publisher."', '".$values['Type']."', '".$values['Status']."', '".$values['Source']."')"; echo $SQL2.'<br/>';
