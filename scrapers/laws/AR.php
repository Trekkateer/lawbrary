<?php //Argentina
    ...
    ]; //TODO: Make it so that it capitalizes words with ( and “
    function mb_ucwordsexcept($str, $delims=' ', $encoding='UTF-8') {global $exceptions; global $capsLock;
        $out = array(trim($str));
    ...
    $saveDate = date('Y-m-d');
    $publisher = '{"es":"Sistema Argentino de Información Jurídica", "en":"The Argentine Legal Information System"}';
    ...
    $subpagesUpdated = array(); //Keeps track of the subpages that have been updated
    for ($offset = $start; $offset <= $limit; $offset += $step) {
        ...
        foreach ($laws as $law) {$law = json_decode($law['documentAbstract'], true)['document'];
            //Gets the province and clears its table if needed
            $LBpage = $provinces[$law['content']['provincia'] ?? $scraper];
            if (!in_array($LBpage, $subpagesUpdated) && $LBpage !== $scraper) {
                $subpagesUpdated[] = $LBpage;//Adds the jurisdiction to the array
            ...
            if (isset($law['content']['titulo-norma'])) {
                if (!str_contains($law['content']['titulo-norma'], ' ') && !in_array($law['content']['titulo-norma'], $fixName)) {
                    //Gets the topic
                    $fixedTopic = str_replace(array_keys($fixTopic), array_values($fixTopic), trim(ucfirst(mb_strtolower($law['content']['titulo-norma'], 'UTF-8')), ' .'));
                        $topic = '\'{"es":"'.$fixedTopic.'", "en":"'.$topics[$fixedTopic].'"}\'';
                } else {
                    $name = $law['content']['titulo-norma'];
                }
            }
            $name = trim(fixQuotes($name ?? $law['content']['titulo_1'] ?? $law['content']['titulo_noticia'] ?? $law['content']['nombre-coloquial'] ?? $law['content']['asunto'] ?? $law['content']['tipo-norma']['texto'].' '.$law['content']['id-infojus'], 'es'), ' .');
            //Gets the type
            ...
            $summary = trim(fixQuotes(isset($law['content']['sumario']) ? (is_array($law['content']['sumario']) ? implode('; ', $law['content']['sumario']):($law['content']['sumario'] ?? NULL)):NULL, 'es'), ' .;');
            $topic = $topic ?? 'NULL';
            ...
            $SQL2 = "INSERT INTO `".strtolower($LBpage)."`(`enactDate`, `enforceDate`, `lastactDate`, `saveDate`, `ID`, `name`, `country`, `regime`, `origin`, `publisher`, `type`, `status`, `summary`, `topic`, `source`) 
                    VALUES ('".$enactDate."', '".$enforceDate."', '".$lastactDate."', '".$saveDate."', '".$ID."', '".$name."', '".$country."', '".$regime."', ".$origin.", '".$publisher."', '".$type."', '".$status."', ".$summary.", ".$topic.", '".$source."')";
            echo '<a href="http://www.saij.gob.ar/busqueda?o='.($offset/$step).'&p='.$step.'&f=Total%7CTipo+de+Documento%2FLegislaci%C3%B3n%7CFecha%7COrganismo%7CPublicaci%C3%B3n%7CTema%7CEstado+de+Vigencia%7CAutor%7CJurisdicci%C3%B3n&s=&v=colapsada">p. '.$offset.'</a>: '.$SQL2.'<br/>';
    ...
    foreach ($subpagesUpdated as $page) {
        $SQL31 = "UPDATE `divisions` SET `lawsUpdated`='".$saveDate."' WHERE `ID`='".strtolower($page)."'"; echo '<br/><br/>'.$SQL31;
