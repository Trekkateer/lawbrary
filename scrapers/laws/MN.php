<?php //TODO: Find a way to get the English name/link as well
    //Settings
    $test = false; $scraper = 'MN';
    $limit = ['mn'=>[], 'en'=>[]]; //Total number of laws desired. Uses $typeID as key

    //Opens the parser (HTML_DOM)
    include '../simple_html_dom.php';

    //Opens my library
    include '../trekkateer_lib.php';
    ...
    function HTTP_Call ($page, $path, $typeID, $code) {
        ...
        return str_get_html(json_decode($response, true)['Html']);
    };
    ...
    $saveDate = date('Y-m-d'); $country = '["MN"]';
    ...
    foreach (array('mn'=>'mn/ajaxList/', /*'en'=>'en/eajaxList/'*/) as $lang => $path) {//Scraping the English page is not necessary
        ...
        foreach ($typeIDs as $type => $typeInfo) {
            //Gets the limit
            $limit = $limit[$lang][$typeInfo[0][0]] ?? explode('/', HTTP_Call(1, $path, $typeInfo[0][0], $typeInfo[0][1])->find('div.shine-huuli-footer', 0)->find('ul', 0)->find('li.number', 0)->plaintext)[1];
            //Loops through the pages
            for ($page = 1; $page <= $limit; $page++) {
                //Processes the data in the table
                $dom = HTTP_Call($page, $path, $typeInfo[0][0], $typeInfo[0][1]);
                ...
                foreach ($laws as $law) {$law = $law->find('div', 0);
                    ...
                    $name = array($lang => fixQuotes(trim($law->find('div[data-block="title"]', 0)->find('a.act-name', 0)->plaintext)));
                    $origin = $typeInfo[1];
                    ...
                    $SQL2 = "INSERT INTO `".strtolower($scraper)."`(`enactDate`, `enforceDate`, `saveDate`, `ID`, `name`, `country`, `regime`, `origin`, `publisher`, `type`, `status`, `source`) 
                                VALUES ('".$enactDate."', '".$enforceDate."', '".$saveDate."', '".$ID."', '".$name."', '".$country."', '".$regime."', ".$origin.", '".$publisher."', '".$type."', '".$status."', '".$source."')";
